...
class TerrainChunkManager {
  ...
  QueueChunk(offset: THREE.Vector2, width: number) {
    ...
    const randomMaterial =
      this.materialArray[Math.floor(Math.random() * this.materialArray.length)];
  ...
  *BuildChunk(chunk: any) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      colours.push(
        this.GenerateColor(chunk, v.x + offset.x, v.z, -v.y + offset.y)
      );
      count++;
  ...
  GenerateHeight(chunk: any, v: THREE.Vector3) {
    ...
    const heightAtVertex = 0; // _noise.simplex(x, y);
