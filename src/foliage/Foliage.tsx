...
/**
 * FOLIAGE — the third spawn class, for mass GPU vegetation.
 *
 * The three classes, by scale and statefulness:
 *   - ACTORS   (src/world/components/Actor.tsx): per-object spawns with
 *     identity/state/interaction — one React component each (beebles,
 *     buildings). Hundreds mounted at once, tops.
 *   - DRESSING (src/dressing/Dressing.tsx): mass stateless rigid scenery —
 ...
 *
 * Foliage deliberately does NOT extend the Dressing base: Dressing's
...
export interface FoliageDefaults {
  /** Max camera distance for the features' chunks (features fall back to
   *  their own defaults when neither prop nor group sets it). */
  renderDistance?: number;
}

const FoliageContext = createContext<FoliageDefaults>({});
 ...
 *   </Foliage>
 */
export const Foliage = ({ children, ...defaults }: React.PropsWithChildren<FoliageDefaults>) => {
  const dataKey = JSON.stringify(defaults);
  const value = useMemo(() => defaults, [dataKey]);
  return <FoliageContext.Provider value={value}>{children}</FoliageContext.Provider>;
};
...
export const useFoliageRenderDistance = (
  ...
  const ctx = useContext(FoliageContext);
