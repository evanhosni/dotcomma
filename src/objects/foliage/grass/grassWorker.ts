...
/** Domain switch (resetDomainSystems): drop the worker so the next GrassField
 *  mount re-inits it with the new world's config. */
export const resetGrassWorker = () => {
  worker?.terminate();
  worker = null;
  initPromise = null;
 ...
 */
export const initGrassWorker = (config: DomainConfig): Promise<void> => {
  if (initPromise) return initPromise;

  worker = new Worker(new URL("../../../utils/workers/grass.worker.ts", import.meta.url), { type: "module" });
