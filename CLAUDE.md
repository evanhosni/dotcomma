...
- **Catch-up must STREAM, never settle a debt.** `generateSpawnPoints` generates at most `MAX_NEW_CHUNKS_PER_REQUEST` (12) new chunks per round-trip, and `ObjectPool` mounts at most `MAX_MOUNTS_PER_BATCH` (8) objects per batch, nearest-first. Chunk keys arrive sorted by distance and are re-sorted against the CURRENT camera position every batch, so ground the player has already left is never generated or mounted at all — instead of being generated, mounted, then swept. Asking for every uncached chunk at once was one multi-second synchronous worker stall (each candidate runs the vertex pipeline) that also held `spawnPending` the whole time, which BLOCKS LOD3-5 terrain building, which keeps stale chunks alive — a genuine feedback loop.
- **Spawning's deference to terrain is time-boxed** (`MAX_FRAMES_DEFERRED_TO_TERRAIN`): a saturated LOD1/2 queue keeps `terrainHighLODPending` true forever, and an indefinitely starved pool never runs its cache eviction and then floods the frame it finally does.
