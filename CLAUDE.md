...
A player moving faster than terrain/spawn generation used to end in single-digit FPS and a minute-long recovery. It was never unbounded cache GROWTH (every cache here is radius- or size-evicted) — it was **unbounded per-batch WORK plus quadratic per-frame bookkeeping**. Four rules keep it from coming back:

- **Catch-up must STREAM, never settle a debt.** Chunk keys go to the worker sorted nearest-first and are re-sorted against the CURRENT camera position every batch, so ground the player has already left is never generated or mounted at all — rather than generated, mounted, then swept. Asking for every uncached chunk at once was one multi-second synchronous worker stall (each candidate runs the vertex pipeline) that also held `spawnPending` the whole time, which BLOCKS LOD3-5 terrain building, which keeps stale chunks alive — a genuine feedback loop.
- **Generation is TIME-budgeted, not count-limited** (`SPAWN_BUDGET_MS`, 100ms). Per-chunk cost varies >10× with terrain — a dense city chunk runs the flatten engine over thousands of pad candidates (~60ms); an empty grassland chunk is nearly free — so any fixed chunk count is simultaneously too long in the city (holding `spawnPending`, and buying no extra throughput once the worker is saturated) and too short in open terrain (leaving the worker idle between batches, since `MIN_FRAMES_BETWEEN_BATCHES` floors the cadence at ~83ms). The worker returns the chunks it FINISHED (`done`); only those are cached, the rest are simply re-requested or dropped. Useful identity: `throughput = N / max(N × cost, 83ms)`, so any budget past the 83ms floor is pure latency.
- **Neither system may starve the other indefinitely.** Both directions are time-boxed: spawning defers to high-res terrain for at most `MAX_FRAMES_DEFERRED_TO_TERRAIN` (an indefinitely starved pool never runs its cache eviction, then floods the frame it finally does), and terrain yields LOD3-5 building to a pending spawn batch for at most `MAX_FRAMES_DEFERRED_TO_SPAWNS` — with back-to-back budgeted batches `spawnPending` is essentially always set, and LOD3-5 chunks are exactly what stale visible chunks wait on to be swapped out.
- **`MAX_MOUNTS_PER_BATCH` (20) can be generous** because mounting cost is not linear in batch size: the nodes in `objectsMapRef` are stable element references, so React bails on unchanged ones and a commit is an O(mounted) key diff, not a re-render; and procedural building geometry / collider creation already spread across frames via a budgeted `TaskQueue`. Setting it too LOW costs an O(mounted) walk per batch to place a handful of objects.
...
- **`TerrainCollider` is memoized on a STABLE descriptor** (`args` / `bodyPosition` built once in `GenerateColliders`). `<HeightfieldCollider>` spreads `args` into the dependency list owning the Rapier shape, so a fresh array or scale object per render removes and rebuilds the heightfield. Every built chunk bumps `colliderVersion` and re-renders the whole list, so unmemoized that was ~50 full heightfield rebuilds (13 of them 97×97) per chunk built — constant while moving fast.
- **Caches store their own coordinates.** The client spawn cache, the spawn worker's chunk cache, and the despawn ledger all carry world-space centers (and spatial-hash membership) on the entry. Eviction sweeps the whole cache every batch; parsing `"cx_cz"` back out of the key made the one operation keeping these bounded the most expensive thing about them.
